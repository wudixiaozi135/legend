package com.view.gameWindow.flyEffect.subs
{
    ...
    public class FlySpecialEffect
    {
         ...
         * @param type 1英雄 2神炉
         * */
        public function FlySpecialEffect(layer:Sprite, type:int)
        {
            this._layer = layer;
            ...
            var endPoint:Point = mc.localToGlobal(new Point(endBtn.x + (endBtn.width >> 1), endBtn.y));//结束点
            ...
            {
                during += 0.2;
                var effect:SingleEffect = new SingleEffect(_layer, during, path, 1, type);
...
class SingleEffect
{
    ...
    private var _effectType:int;//1英雄  2神炉
    public function SingleEffect(layer:Sprite, during:Number, path:Array, alpha:int, effectType:int)
    {
        _layer = layer;
        _during = during;
        _path = path;
    ...
    public function startPlay(start:Point, end:Point):void
    {
        ...
        if (_effectType == 1)
        {
            url = EffectConst.RES_FIRE_DRAGON_STRENGTH;
        } else if (_effectType == 2)
        {
            url = EffectConst.RES_BLUE_GREEN;
        }
        _effect = new UIEffectLoader(_effectContainer, 0, 0, .8, .8, url);
