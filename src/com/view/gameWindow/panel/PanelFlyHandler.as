package com.view.gameWindow.panel
{
	...
	public class PanelFlyHandler
	{
		public function PanelFlyHandler()
		{
			mountPanels=new Vector.<PanelBase>();
//			handler=new Vector.<PanelBase>();
		}
		
		private var mountPanels:Vector.<PanelBase>;
		
		private static var _instance:PanelFlyHandler;
		private var doing:Boolean=false;
		private var handler:Vector.<PanelBase>;
		
		public static function getInstance():PanelFlyHandler
		{
			if(_instance==null)
			{
				_instance=new PanelFlyHandler();				
			}
			return _instance;
		}
		 ...
		 */		
		public function panelRunHideMount(panel:PanelBase):void
		{
			mountPanels.splice(mountPanels.indexOf(panel),1);
			/*trace(panel.name+"-mount，count"+mountPanels.length);*/
			doMount();
			if(panel.moveing&&panel.isSingleton)
			{
				TweenLite.killTweensOf(panel);
				onCloseComple(panel);
			}
			panel.moveing=true;
			TweenLite.to(panel,0.5,{alpha:0,x:panel.openX,y:panel.openY,scaleX:0.1,scaleY:0.1,onComplete:onCloseComple,onCompleteParams:[panel]});
		}
		
		private function onCloseComple(panel:PanelBase):void
		{
			panel.moveing=false;
			panel.isSingleton ? panel.hide() : panel.destroy();
		}
		
		private function doMount():void
		{
