package com.view.gameWindow.panel.panels.openGift.journeyReward
{
	...
	public class JourneyRewardViewHandler implements IScrollee
	{
		private var skin:McOpenJourney;
		private var panel:JourneyReward;
		private var _vec:Vector.<JourneyRewardItem>;
		private var cfgVec:Vector.<OpenServerJourneyRewardCfgData>;
		private var _scrollBar:ScrollBar;
		private var _scrollRect:Rectangle;
		internal var _linkText:LinkText;
		private var _uiCenter:UICenter;
		public function JourneyRewardViewHandler(p:JourneyReward)
		{
			panel = p;
			skin = panel.skin;
			initData();
		...
		private function initData():void
		{
			...
			_linkText.init(CfgDataParse.pareseDesToStr(cfg.open_server_journey_des));
			...
			_vec = new Vector.<JourneyRewardItem>();
			cfgVec = ConfigDataManager.instance.OpenServerJourneyRewardNumByDay(OpenServiceActivityDatamanager.instance.curDay);
			cfgVec.sort(sortIndex);
			for(var i:int = 0;i<cfgVec.length;i++)
			{
				var item:JourneyRewardItem = new JourneyRewardItem();
		...
		private function sortIndex(a:OpenServerJourneyRewardCfgData,b:OpenServerJourneyRewardCfgData):int
		{
			if(a.index < b.index)
			{
				return -1;
