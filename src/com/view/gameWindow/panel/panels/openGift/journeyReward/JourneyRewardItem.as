package com.view.gameWindow.panel.panels.openGift.journeyReward
{
	...
	public class JourneyRewardItem extends Sprite
	{
		...
		public function setData(_index:int):void
		{
			...
			index = _index;
			cfg = ConfigDataManager.instance.OpenServerJourneyRewardData(day,index);
		...
		public function refreshData():void
		{
			...
			var dailyData:OpenJourneyData = OpenServiceActivityDatamanager.instance.journeyData[day];
			var isget:Boolean = Boolean(dailyData.data[index].state);
			var param:int = dailyData.data[index].num;
			var num:int;
			if(cfg.type == OpenJourneyData.ROLE_LV || cfg.type == OpenJourneyData.HERO_LV)
			{
				num = cfg.level;
			}
			else
			...
			if(day != openDay&&cfg.type==OpenJourneyData.REPUTION_INTERGAL)
			{
				skin.txtInfo.htmlText = cfg.name;
		...
		protected function onClick(event:MouseEvent):void
		{
			...
			if(event.target == skin.btnGet)
			{
				var day:int = OpenServiceActivityDatamanager.instance.curDay;
				var openday:int = WelfareDataMannager.instance.openDay+1;
				if(day!=openday&&cfg.type==7)
				{
					Alert.message(StringConst.PANEL_OPEN_GIFT_021);
				}
				else
				{
					OpenServiceActivityDatamanager.instance.getJourneyReward(index);
					storeBitmaps();	
				}
			}
		}
