package com.view.gameWindow.panel.panels.boss.outside
{
	...
	public class TabOutSideScrollRect implements IScrollee
	{
		...
		public function TabOutSideScrollRect(parent:TabOutsideViewHandle,skin:MCFieldBoss,rsloader:RsrLoader)
		{
			_handler = parent;
			_rsloader = rsloader;
			_skin = skin;
		...
		public function refresh(data:Vector.<BossCfgData>):void
		{
			...
			if(_items.length == data.length)
			{
				for(var i:int = 0;i<_items.length;i++)
				{
					...
					item = _items[i] as OutsideBossItem;
			...
			else
			{
				...
				for (var j:int = 0;j<data.length;j++)
				{
					...
					item = new OutsideBossItem(_handler);
					item.init(_rsloader);
					item.skin.x = 5;
